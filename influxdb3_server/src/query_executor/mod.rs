...
impl QueryExecutorImpl {
    ...
    pub(crate) fn new_df_session_context(&self, db_name: &str) -> Result<Arc<datafusion::execution::context::SessionContext>, QueryExecutorError> {
        let db_schema_arc = self.catalog.db_schema(db_name)
            .ok_or_else(|| QueryExecutorError::DatabaseNotFound { db_name: db_name.to_string() })?;
        ...
        let catalog_provider = Arc::new(Database::new(CreateDatabaseArgs {
            db_schema: Arc::clone(&db_schema_arc), // Clone Arc for this usage
            ...
            system_schema_provider: Arc::new(SystemSchemaProvider::AllSystemSchemaTables(
                AllSystemSchemaTablesProvider::new(
                    Arc::clone(&db_schema_arc), // Use the already fetched db_schema_arc
        ...
        let mut cfg = self.exec.new_session_config()
            .with_default_catalog(catalog_provider);
