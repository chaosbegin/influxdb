...
use datafusion::execution::context::SessionConfig; // For creating SessionConfig
use datafusion::prelude::Extensions; // For ExtensionMap
...
impl DistributedQueryService for DistributedQueryServerImpl {
    ...
    async fn execute_query_fragment(
        ...
        // Create SessionConfig and add ShardIdFilterExtension
        let mut session_config = SessionConfig::new();
        for (k,v) in req.session_config {
            session_config.set(&k, &v) // Use SessionConfig's setter
                .map_err(|e| Status::invalid_argument(format!("Failed to set session config {}: {}", k, e)))?;
        }

        let mut extensions = Extensions::new();
        if let Some(shard_id_val) = req.shard_id {
            // Assuming 0 is not a valid shard_id or means "no filter" if proto uses uint64 default.
            // For this example, any non-zero shard_id will set the filter.
            // If shard_id is inherently Option<u64> in proto, this check might be different.
            // Given proto definition `optional uint64 shard_id = 3;`, if it's present, it's Some(val).
            extensions.insert(ShardIdFilterExtension(Some(ShardId::new(shard_id_val))));
            observability_deps::tracing::info!("Applying ShardIdFilterExtension: {:?}", shard_id_val);
        } else {
            extensions.insert(ShardIdFilterExtension(None)); // Explicitly no filter
            observability_deps::tracing::info!("No ShardId specified in request, not applying filter.");
        }

        // Create IOxSessionContext with the extensions
        // new_context_for_db likely needs to be adapted or a new method used if it doesn't take extensions/config directly.
        // For now, assume QueryExecutor provides a way to create a context with a given SessionConfig.
        // Let's assume new_context_for_db takes QueryConfig which is part of SessionConfig.
        // This part is a bit hand-wavy as it depends on QueryExecutor's exact API.
        // A common pattern is ctx.with_state_config(updated_session_state_config).
        // For simplicity, if new_context_for_db doesn't take SessionConfig with extensions,
        // we'll try to update the context after creation if possible.
        // Most SessionContext implementations allow access to inner state for modification or are created with config.

        // Placeholder: Assume we get a default context and then try to apply extensions.
        // This is not ideal. Ideally, extensions are part of initial config.
        // If QueryExecutor::new_context_for_db cannot take a full SessionConfig with extensions,
        // this part of the plan (Option 1 for RemoteScanExec) is harder.
        // For now, let's proceed assuming we can create a context and then set extensions if needed,
        // or that the QueryConfig within it can be updated.

        let session_ctx = self.query_executor.new_context_for_db(
            &req.db_name,
            None, // span context
            Some(&iox_query::exec::QueryConfig::new().with_extensions(extensions)) // Pass QueryConfig with extensions
        )
